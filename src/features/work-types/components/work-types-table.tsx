...
interface WorkTypesTableProps {
  ...
  onEdit: (workType: WorkTypeDto) => void;
  onDelete: (workType: WorkTypeDto) => void;
}

export function WorkTypesTable({ workTypes, onEdit, onDelete }: WorkTypesTableProps) {
  if (workTypes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-1 rounded-lg border border-dashed border-border py-16 text-center">
        <p className="text-sm font-medium text-foreground">Niciun tip de lucrare încă</p>
        <p className="text-sm text-muted-foreground">Adaugă primul tip cu butonul de mai sus.</p>
      </div>
    );
  }
            ...
            <tr key={workType.id} className="border-b border-border last:border-0 hover:bg-accent/40">
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => onDelete(workType)} aria-label="Șterge">
